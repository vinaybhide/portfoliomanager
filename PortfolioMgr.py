...
class PortfolioManager:
    ...
    def OnTreeSingleClick(self, event):
        ...
        self.output_tree.after(300, self.callSingleDoubleClick, event)
    ...
    def __init__(self):
        ...
        #scroll bar for Tree
        self.vert_scroll = ttk.Scrollbar(self.content, orient=VERTICAL, command=self.output_tree.yview)
        self.output_tree.configure(yscrollcommand=self.vert_scroll.set)
        self.horiz_scroll = ttk.Scrollbar(self.content, orient=HORIZONTAL, command=self.output_tree.xview)
        ...
        self.horiz_scroll.grid(row=2, column=0, columnspan=11, sticky=(N, E, W, S))
